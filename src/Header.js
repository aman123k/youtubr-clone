import React, { useState } from "react";
import { HiMicrophone } from "react-icons/hi";
import { BiVideoPlus } from "react-icons/bi";
import { IoIosSearch } from "react-icons/io";
import { AiOutlineArrowLeft } from "react-icons/ai";
import { IoMdNotificationsOutline } from "react-icons/io";
import { Link } from "react-router-dom";
import { IoIosMenu } from "react-icons/io";
import Slidebar from "./Slidebar";
function Header() {
  const [sidebar, setSidebar] = useState(false);
  const [search, setSearch] = useState(false);
  const [value, setValue] = useState([]);
  return (
    <>
      <header className=" sticky top-0 bg-white flex items-center z-50 max-[800px]:px-3">
        <div className=" w-[7.5%] items-center ">
          <Link to="#" onClick={() => setSidebar(!sidebar)}>
            <IoIosMenu
              className={` w-full text-3xl text-black text-center block
              ${search ? "hidden" : "block"}`}
            />
          </Link>
          <AiOutlineArrowLeft
            onClick={() => setSearch(!search)}
            className={` w-full text-2xl text-black text-center cursor-pointer
             ${search ? "block" : "hidden"}`}
          />
        </div>
        {/* header */}
        <section
          className=" flex w-[90%] justify-between
         max-[500px]:w-[93%] items-center 
        border-b "
        >
          {/* left container */}
          <div className=" flex items-center">
            <Link to="/">
              <img
                src="data:image/png;base64,"
                alt="youtube"
                className={`${
                  search ? "max-[800px]:hidden" : "max-[800px]:block"
                }  
                h-[55px]`}
              />
            </Link>
          </div>
          {/* meddle container */}
          <div
            className={`w-[45%] flex gap-[5%] items-center 
            ${
              search
                ? "max-[800px]:w-full max-[800px]:sticky max-[800px]:my-2"
                : "max-[800px]:hidden"
            }  
         `}
          >
            <div
              className=" w-[90%] border rounded-3xl 
            items-center flex h-[35px] relative overflow-hidden bg-white "
            >
              <input
                type="text"
                placeholder="Search"
                onChange={(e) => setValue(e.target.value)}
                value={value}
                className="flex-1 outline-0
                 text-[16px] h-[100%] px-[15px] 
                 max-sm:flex-none max-sm:w-[85%]
               "
              />
              <div
                className=" cursor-pointer
                 h-[100%] text-[20px] border-l
                border-[#e5e5e5] px-[14px] 
                py-1.5 bg-[#f9f9f9]
                max-[500px]:absolute right-0
               "
              >
                <IoIosSearch />
              </div>
            </div>
            <HiMicrophone
              className=" text-[25px] 
            w-[40px] p-[10px] rounded-full 
            h-[40px] bg-[#f9f9f9] cursor-pointer"
            />
          </div>

          {/* left container */}
          <div
            className={`flex items-center gap-5 max-[800px]:gap-3
           ${search ? "max-[800px]:hidden" : ""}   
          `}
          >
            <IoIosSearch
              className={` cursor-pointer 
              text-[25px] hidden max-[800px]:block `}
              onClick={() => setSearch(!search)}
            />
            <HiMicrophone
              className="text-[25px] hidden h-[21px] 
            cursor-pointer max-[800px]:block"
            />
            <BiVideoPlus className=" text-[25px] cursor-pointer" />
            <IoMdNotificationsOutline className=" text-[25px] cursor-pointer" />
            <img
              src="https://png.pngtree.com/png-clipart/20220909/original/pngtree-cartoon-man-avatar-vector-ilustration-png-image_8515463.png"
              className=" h-[25px] cursor-pointer"
              alt="avatar"
            />
          </div>
        </section>
        {/* slide bar menu */}
        {/* slidebar menu background */}
        <div
          className={`fixed flex top-0 w-full bg-[#191919] 
          opacity-40 h-screen backdrop-opacity-10
            ${sidebar ? "left-0" : "left-[-100%]"} `}
        ></div>
        {/* slide menu */}
        <div
          className={`w-[14rem] bg-white absolute top-0 
         h-screen z-100 duration-700 2xl:w-2/12 ease-in-out
              ${sidebar ? "left-0" : "left-[-100%]"}`}
          onClick={() => setSidebar(!sidebar)}
        >
          <div
            className=" flex items-center
             w-full gap-3 justify-center bg-"
          >
            <Link to="#" onClick={() => setSidebar(!sidebar)}>
              <IoIosMenu className=" w-full text-3xl text-black text-center" />
            </Link>
            <Link to="/">
              <img
                src="data:image/png;base64,"
                alt="youtube"
                className=" h-[55px] mr-9"
              />
            </Link>
          </div>
          <Slidebar />
        </div>
      </header>
    </>
  );
}

export default Header;
